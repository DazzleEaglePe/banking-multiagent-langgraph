...
| **Observabilidad / Tracing** | **Caja Negra.** Si el bot falla o entra en bucle, los desarrolladores no tienen trazas del prompt o del nodo que falló. | **Transparencia Total.** Registro de cada paso, latencias y consumo de tokens en vivo con **LangSmith**. |
...
5. **Bucle de Estado Infinito (State Trapping / Bucle de Callejón Sin Salida):**
   * **Problema:** Al ingresar al flujo de validación de identidad (`1. Mis Cuentas de Ahorros`), el chatbot exige ingresar un DNI. Si el usuario intenta salir escribiendo palabras clave lógicas de escape como `"MENU"`, `"menu"`, `"inicio"`, `"cancelar"` o `"atras"`, el bot las rechaza por completo arrojando *"No he entendido su solicitud..."* y repitiendo el prompt de DNI.
   * **Impacto:** El usuario queda completamente "secuestrado" dentro del sub-flujo sin ninguna salida por comando de texto, obligándolo a esperar los 5 minutos de inactividad para que la sesión expire. En una arquitectura con **LangGraph**, el **Router** evalúa globalmente cada entrada en cada turno, permitiendo romper el contexto local si el usuario expresa una intención de cancelación o cambio de tema.
...
7. **Nula Observabilidad y Tracing (Caja Negra vs. LangSmith):**
   * **Problema:** Si Boty falla (como cuando repite *"no he entendido su solicitud"* en bucle), los desarrolladores del banco no tienen un registro de cuál nodo del backend falló ni qué inputs exactos provocaron la caída.
   * **Impacto:** Dificultad para depurar errores en producción y optimizar prompts. Nuestra arquitectura integra **LangSmith** nativamente, permitiendo visualizar el grafo en tiempo real, medir la latencia de cada agente, rastrear el consumo de tokens y depurar llamadas fallidas de Gemini de manera instantánea.
...
Aquí te comparto los pilares técnicos de la solución:
...
Encontré que el bot actual expone claves y DNI en texto plano en la ventana de chat y se congela en bucles infinitos de error. Para solucionarlo, implementé un nodo auditor que actúa como "LLM-as-a-Judge" para evitar fugas de información y derivar de inmediato a canales humanos ante emergencias.

4. Observabilidad y Monitoreo de Producción (LangSmith) 👁️
En lugar de operar a ciegas como una "caja negra", el sistema integra tracing asíncrono con LangSmith. Esto nos permite visualizar el grafo en vivo, inspeccionar la latencia de cada agente, rastrear el conteo de tokens exacto de Gemini y depurar prompts directamente en producción.

5. RAG Híbrido y Semántico en Microsoft Azure (AI-103) ☁️
Diseñé e implementé un pipeline corporativo utilizando los SDKs oficiales de Azure AI Search y Azure OpenAI. Integra búsquedas híbridas (Keyword BM25 + Vectores) y reordenamiento semántico por relevancia (Semantic Reranker), garantizando precisión y seguridad a nivel empresarial de acuerdo al nuevo estándar de certificación AI-103 (Azure AI Apps and Agents Developer).

Orquestar agentes de IA no es solo conectar prompts; es darles memoria (usando MemorySaver para el seguimiento de hilos de chat), dotarlos de herramientas y establecer observabilidad robusta. 

¡El código fuente completo y la suite de evaluación ya están disponibles en mis repositorios! 
...
👉 Módulo 2.1 (RAG Corporativo en Azure): https://github.com/DazzleEaglePe/azure-search-openai-rag
...
#ArtificialIntelligence #AzureAI #LangGraph #AIArchitecture #Microfinanzas #SoftwareEngineering #GenerativeAI #Python
```
